...
exports.init = function (startupTime, config, events, logger) {
  ...
  config.elasticsearchUrl = config.elasticsearchUrl || 'http://elasticsearch:9200';
    ...
    .then(function () {
      ...
      events.on('flush', function (timeStamp, metrics) {
        ...
        Object.keys(metrics.counter_rates).forEach(function (name) {
          ...
          var type = 'counter';
          storeMetric(config, logger, timestamp, name, value, type);
        });
...
function storeMetric(config, logger, timestamp, name, value, type) {
